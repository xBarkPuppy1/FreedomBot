...
public class Command_getgamemode implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(BotUtil.BOTPREFIX + "You need to state a player's username to see their gamemode!");
            return true;
        } else {
            final Player player = getPlayer(args[0]);

            sender.sendMessage(BotUtil.BOTPREFIX + player.getName() + "'s current gamemode is " + player.getGameMode() + ".");
        }
